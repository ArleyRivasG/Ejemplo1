...
def buscarMenor(mat):
    menor=mat[0,1]
    mat = np.array(mat) #opcional para volverlo matriz si no lo es
    for i in range(len(mat)):
        for j in range(len(mat[i])):
            if ((i+j % 2) != 0 and menor > (i+j) ):
                menor = mat[i,j]

    return menor
    print(menor)

list_1 = [[1,2,3,6],[4,5,6,5], [23,10,7,8], [2, 4, 7, 8]]
mat_1 = np.array(list_1)
print(list_1 , "\n" , mat_1)

buscarMenor(mat_1) #~Funciona bien, devuelve el menor encontrado
...
print("===TU SALIDA===:\nMenor = ", menor_estudiante, "\nPosiciones donde está el menor = ",
      posiciones_menor_estudiante, "\n")
